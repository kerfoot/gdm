...
function error_msg() {
    ...
    ms=$(date +%N);
    echo "$(date +"%Y-%m-%d %H:%M:%S,${ms:0:3}"):$app:ERROR:$msg [Line $BASH_LINENO]" >&2;
}
...
#    echo "$(date --utc +%Y-%m-%dT%H:%M:%SZ):$app:[Line $BASH_LINENO]:WARN:$msg" >&2;
    ms=$(date +%N);
    echo "$(date +"%Y-%m-%d %H:%M:%S,${ms:0:3}"):$app:WARN:$msg [Line $BASH_LINENO]" >&2;
}
...
#    echo "$(date --utc +%Y-%m-%dT%H:%M:%SZ):$app:[Line $BASH_LINENO]:INFO:$msg" >&2;
    ms=$(date +%N);
    echo "$(date +"%Y-%m-%d %H:%M:%S,${ms:0:3}"):$app:INFO:$msg [Line $BASH_LINENO]" >&2;
}
# Message function. Prints to STDOUT
function debug_msg() {
    if [ -z "$1" ]
...
#    echo "$(date --utc +%Y-%m-%dT%H:%M:%SZ):$app:[Line $BASH_LINENO]:DEBUG:$msg" >&2;
    ms=$(date +%N);
    echo "$(date +"%Y-%m-%d %H:%M:%S,${ms:0:3}"):$app:DEBUG:$msg [Line $BASH_LINENO]" >&2;
}
